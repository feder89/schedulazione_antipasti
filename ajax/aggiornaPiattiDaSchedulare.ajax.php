<?php
    ...
    if($date <=0){
		...
		if(isset($_POST["piatti"])){
			...
			$id_prog;
			$query_idprog="SELECT MAX(idprogrammazione) as idprog FROM programmazioneordini WHERE serata='$date'";
			...
			foreach ($portate as $key=>$p) {
				...
				while($i<=$p["quant"]){
					...
					$query = 	"UPDATE programmazioneordini SET stato=2 , idprogrammazione=$id_prog
								...
								AND prog.portata=$piatto AND stato=1 AND serata='$date' LIMIT 1);" ;
