<?php
    ...
    if($date <=0){
		...
		$query="SELECT *,COUNT(id) AS nr FROM programmazioneordini 
				WHERE stato=3 and categoria IN ('antipasto','bruschette e crostoni','dolce') AND serata='$date'
				GROUP BY portata, tavolo ,indice, idprogrammazione
				ORDER BY idprogrammazione DESC, tavolo, portata,FIELD(categoria, 'antipasto','bruschette e crostoni','dolce')";			
