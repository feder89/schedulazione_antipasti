<?php
    ...
    if($date <=0){
		...
		if(isset($_POST['tavolo'])  && isset($_POST['indice'])){
			...
			WHERE tavolo=$tavolo AND indice=$indice AND stato < 2 and categoria IN ('antipasto','bruschette e crostoni','dolce') AND serata='$date'
			GROUP BY portata
			ORDER BY FIELD(categoria, 'antipasto','bruschette e crostoni','dolce')";
